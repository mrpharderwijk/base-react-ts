...
const MainUserMenu: React.FC<MainUserMenuProps> = ({ isLoggedIn }: MainUserMenuProps) => {
  ...
  return (
    <div className="main-user-menu relative">
      {isLoggedIn ? (
        <>
          <Button label="Mr. Anonymous" userAvatar onClick={() => toggleSubMenu()} />
          {isSubMenuActive ? (
